...
页面分为四个入口：`/` 总览；`/settings` 修改连接信息和管理员账户；`/users` 已登记及已绑定用户列表，可修改、删除本平台记录；`/guide` 使用教程。旧 OneBot 与 MCSManager 配置不再使用，新 Bot 凭证需重新填写。
...
QQ 群内命令：`/register <QQ号>`、`/bind <玩家名>`、`/motd`。用户先在群里 @机器人发送 `/register 36000000`（换成自己的数字 QQ 号），机器人回显号码并给出 `BIND-XXXXXX`。同一 OpenID 的用户在同群 5 分钟内回复该码后自动登记，不需管理员审核。**这只二次确认填写的号码，不能证明号码归属，也不会执行玩家绑定。**登记后再发 `/bind <自己填写的玩家名>`，此时才通过 RCON 执行 `aqqbot whitelist bind <登记QQ号> <玩家名>`。RCON 输出会显示在回复中，但真实绑定结果仍以 AQQBot/服务器状态为准。按用户要求，不提供聊天日志查询。
