...
随后填写 RCON 主机、端口和密码；Minecraft 主机/游戏端口；QQ 官方 Bot 的 AppID、AppSecret 和允许使用的群 OpenID。若要把 MC 玩家聊天推送到 QQ，还要填写本程序可读取的 `logs/latest.log` 完整路径；QQ 群聊天转发到 MC 则使用 RCON。不需要 MCSManager API。请不要把管理网页、`data/`、密码或 AppSecret 暴露到公网。RCON 需在 MC 服务端启用，并设置强密码。
