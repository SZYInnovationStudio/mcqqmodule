const keys = ['mcsmUrl', 'mcsmApiKey', 'daemonId', 'instanceId', 'rconHost', 'rconPort', 'rconPassword', 'mcHost', 'mcPort', 'onebotWsUrl', 'onebotToken', 'allowedGroups'];
const secrets = ['mcsmApiKey', 'rconPassword', 'onebotToken'];
...
export function validateConfig(input, current = {}) {
  ...
  next.onebotWsUrl = endpoint(next.onebotWsUrl, ['ws:', 'wss:']);
  ...
  if (next.allowedGroups && !/^\d{5,20}(,\d{5,20})*$/.test(next.allowedGroups)) throw new Error('群号列表格式不合法');
  return next;
}
