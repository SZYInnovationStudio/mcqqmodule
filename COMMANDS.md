...
聊天互通：允许群的普通文本会通过 `tellraw @a` 显示在 MC 中，`[QQ群]` 为绿色，其余文字使用默认颜色；机器人自身消息及斜杠命令不转发。MC → QQ 需要在后台填写本程序可读取的 `logs/latest.log` 完整路径，只转发新出现的玩家聊天到全部允许群，格式为 `[服务器] 玩家名:内容`。这是 QQ 官方 Bot 的主动群消息，会受到官方额度/权限限制。若 MC 在另一台服务器且没有共享日志，RCON 本身不能提供聊天流。
