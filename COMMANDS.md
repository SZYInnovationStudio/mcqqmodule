...
聊天互通由后台两个独立开关控制，不影响上面的业务指令。它们直接修改 MC 服务器正在使用的 AQQBot `config.yml` 中 `chat.group_to_server.enable` 与 `chat.server_to_group.enable`，并将 `messages.yml` 的 `qq.chat_from_game` 设置为 `[服务器] ${player}: ${message}`、`game.chat_from_qq` 设置为 `&a[QQ群]&r ${userName}: ${message}`。保存时备份原文件，并通过 RCON 执行 `aqqbot reload`。本平台不再读 `logs/latest.log`，也不再用 `tellraw` 重复转发普通群聊。必须填写程序可读写的**服务器实际插件文件路径**；下载到本机的副本不能控制远程插件。聊天实际收发由 AQQBot 自身的 OneBot 后端完成。
