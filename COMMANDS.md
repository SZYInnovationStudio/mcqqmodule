...
聊天互通由后台两个独立开关控制，默认都关闭；它们不影响上面的业务指令。开启 QQ → MC 后，允许群中机器人实际收到的普通文本会通过 `tellraw @a` 显示在 MC 中，`[QQ群]` 为绿色，其余文字使用默认颜色；机器人自身消息及斜杠命令不转发。发送者名称取 QQ 官方事件提供的名称，若平台未提供则使用已登记 QQ 号或「群友」。要接收非 @ 的全部群聊，QQ 侧须允许机器人接收全量群消息；若只收到 @事件，程序也只能转发 @消息。MC → QQ 需要开启对应开关，并在后台填写本程序可读取的消息文件完整路径；目前按 `logs/latest.log` 格式读取，待核对服主提供的实际文件。只转发新出现的玩家聊天到全部允许群，格式为 `[服务器] 玩家名:内容`。这是 QQ 官方 Bot 的主动群消息，会受到官方额度/权限限制。若 MC 在另一台服务器且没有共享日志，RCON 本身不能提供聊天流。
